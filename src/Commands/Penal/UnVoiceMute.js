...
module.exports.execute = async (client, message, args) => {
    ...
    if (penalId) {
        ...
        message.reply(`<:Unmuted:815668868872667166> ${member ? member.displayName : victim.username}(${victim.id}) kullanıcısının \`#${penalId}\` numaralı sesli sohbet cezasını kaldırdın. ✅`);
    }
    else message.reply(`${member ? member.displayName : victim.username}(${victim.id}) sesli susturma cezası kaldırma işlemi iptal edildi.`);
};
